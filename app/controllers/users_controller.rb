class UsersController < ApplicationController
  ...
  def search
    from_friends = Friend.where(from_friend_id: current_user.id).where(status: 2).map { |e| e.to_friend_id }
    to_friends = Friend.where(to_friend_id: current_user.id).where(status: 2).map { |e| e.from_friend_id }
    from_friends_request = Friend.where(from_friend_id: current_user.id).where(status: 1).map { |e| e.to_friend_id }
